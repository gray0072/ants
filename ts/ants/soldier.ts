...
import { STATE, Ant, AntState } from '../state';
import { MapModule } from '../map';
import { nearestVisibleEnemy, requestPath, requestPathSmart, followPath, reveal, dist } from '../ant';

export function updateSoldier(ant: Ant): void {
  reveal(ant);
  ...
  // Patrol near nest — only target passable cells
  ant.state = 'patrol';
  if (!ant.path?.length) {
